...
- Извлекает вехи и задачи из заданных вкладок.
- Рассчитывает длительность по цветовой заливке ячеек (от колонки AM).
- Игнорирует автоматические поля дат.
- Учитывает производственный календарь РФ.
- Формирует Excel с шириной столбцов Ганта = 4 и шрифтом = 9.
""")
...
def get_ru_holidays(years_range):
    ...
    try:
        ...
        # Fallback на случай проблем с библиотекой holidays
        ru_holidays = set()
        for year in years_range:
            ru_holidays.update([
                ...
                datetime(year, 2, 23).date(), datetime(year, 3, 8).date(),
...
def count_colored_weeks(ws, row_idx, start_col="AM", end_col="ZZ"):
    ...
    for col_idx in range(start_col_idx, end_col_idx + 1):
        ...
        if fill and fill.fgColor:
            ...
            # Исключаем прозрачный и белый цвета
            if rgb_str not in ['00000000', '00FFFFFF', 'FFFFFF', 'FFFFFFFF', 'NONE', '']:
                # Дополнительная проверка: ячейка не должна быть абсолютно пустой (защита от артефактов)
...
with col2:
    project_start = st.date_input("📅 Дата начала проекта:", value=datetime.now())
        ...
        try:
            ...
            for sheet_name in phase_sheets:
                if sheet_name in wb.sheetnames:
                    ...
                    header_row = 1 # Упрощенно считаем, что данные идут после 1 строки
                    
                    for row in range(header_row + 1, min(ws.max_row + 1, 1000)):
                        ...
                        if desc_val and str(desc_val).strip():
                            ...
                            if duration == 0:
                                duration = 1 # Минимальная длительность, если текст есть, а цвета нет
            ...
            # ============================================================
            # 4. ГЕНЕРАЦИЯ ИТОГОВОГО EXCEL ФАЙЛА С ДИАГРАММОЙ ГАНТА
            # ============================================================
            ...
            # Цвета
            gantt_fill = PatternFill(start_color="4472C4", end_color="4472C4", fill_type="solid") # Синий
            header_fill = PatternFill(start_color="2F5496", end_color="2F5496", fill_type="solid") # Темно-синий
            holiday_fill = PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid") # Светло-красный
            milestone_fill = PatternFill(start_color="FFC000", end_color="FFC000", fill_type="solid") # Желтый
            ...
            for w in range(1, max_weeks + 1):
                ...
                # СТРОГО ПО ТЗ: ширина столбцов диаграммы = 4
            ...
            for idx, task in enumerate(all_tasks, start=2):
                ...
                for w in range(task["Duration Weeks"]):
                    ...
                    if col_idx <= first_week_col + max_weeks - 1:
                        ...
                        cell.value = "█" # Символ для наглядности в узкой колонке
                        cell.fill = gantt_fill
                        cell.font = Font(size=9, color="FFFFFF", name="Arial") # Белый шрифт 9
                        cell.alignment = Alignment(horizontal="center", vertical="center")
                        cell.border = thin_border
                
                # Границы для основных ячеек
                ...
                # Сдвиг даты для следующей задачи (последовательное выполнение)
                current_date = end_dt

            # Добавление вех в конец таблицы (опционально, для полноты картины)
            if milestones:
                m_row = len(all_tasks) + 3
                out_ws.cell(row=m_row, column=2, value="КЛЮЧЕВЫЕ ВЕХИ ПРОЕКТА").font = Font(size=10, bold=True, name="Arial")
                for i, m in enumerate(milestones[:10]): # Ограничим 10 вехами для компактности
                    r = m_row + 1 + i
            ...
            out_ws.freeze_panes = "C2" # Закрепить первые два столбца
